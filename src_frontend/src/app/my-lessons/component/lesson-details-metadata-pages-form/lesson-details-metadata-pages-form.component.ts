...
@Component({
    ...
    template: `
        <div *ngFor="let pageData of data" class="wrapper">
...
export class LessonDetailsMetadataPagesFormComponent implements OnChanges {
    @ViewChildren(LessonDetailsMetadataBaseFormComponent)
    ...
    public save(): Observable<Metadata[]> {
        return forkJoin(
            this.metadataComponents.map((item: any) => item.save())
        ) as Observable<Metadata[]>;
    }

    public reset() {
        this.data = this.pages.map((page: any) =>
            Metadata.fromLessonPage(page)
        );
