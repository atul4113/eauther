...
import {
    ILessonRaw, IMyAddonsRaw, IMyLessonsRaw, Lesson, LessonPage, LessonProperties,
    LessonsOrder, Metadata
} from '../model/lesson';
...
export class MyContentService {
    ...
    public getLessonDetails (id: number): Observable<Lesson> {
        return this._restClient.get(`${MY_CONTENT_URL}${id}`).pipe(
    ...
    public getLessonMetadata (lessonId: number): Observable<Metadata> {
        ...
        return this._restClient.get(url).pipe(
            map((response: ILessonRaw) => Metadata.fromLesson(new Lesson(response)) )
        );
    }

    public updateLessonMetadata (lesson: Lesson, metadata: Metadata): Observable<any> {
        ...
        const data = {
            ...
            'tags': metadata.tags,
            'custom_values': metadata.customValues.map(custom => custom.raw(true))
        };

        return this._restClient.put(url, data);
    }

    public updateLessonProperties (lesson: Lesson, properties: LessonProperties): Observable<any> {
        const data = {
    ...
    public getLessonHistory (lessonId: number): Observable<FileStorage[]> {
      return this._restClient.get(MY_CONTENT_URL + lessonId + '/history').pipe(
          map(response => {
            return response.map(fileStorage => new FileStorage(fileStorage));
          })
      );
    }
    ...
    public updateLessonPagesMetadata (lesson: Lesson, pages: LessonPage[]): Observable<LessonPage[]> {
        const url = `${MY_CONTENT_URL}${lesson.id}/pages_metadata`;
        const data = pages.map(page => page.raw());

        return this._restClient.put(url, data).pipe(
            map(response => response.map(page => new LessonPage(page)))
        );
    }
    ...
    public getLessonBugs (lessonId: number): Observable<Bug[]> {
        ...
        return this._restClient.get(url).pipe(
            map(response => response.map(bugRaw => new Bug(bugRaw)))
        );
    }
    ...
    public getLessons (cursor: string = null, spaceId): Observable<{ lessons: Lesson[], cursor: string, moreCount: number }> {
        const requestUrl = spaceId ? MY_CONTENT_URL + spaceId + '/lessons' : LESSONS_URL;

        return this._restClient.get(requestUrl + (cursor ? `?cursor=${cursor}` : '')).pipe(
            map(response => {
               const data = <IMyLessonsRaw> response;
               const lessons = data.lessons.map(lesson => new Lesson(lesson));
    ...
    public getTrash (cursor: string = null, spaceId): Observable<{ lessons: Lesson[], cursor: string, moreCount: number }> {
        const requestUrl = spaceId ? MY_CONTENT_URL + spaceId + '/trash' : TRASH_URL;
    ...
    public getAddons (cursor: string = null, spaceId): Observable<{ addons: Lesson[], cursor: string, moreCount: number }> {
        const requestUrl = spaceId ? MY_CONTENT_URL + spaceId + '/addons' : LESSONS_URL;

        return this._restClient.get(requestUrl + (cursor ? `?cursor=${cursor}` : '')).pipe(
            map(response => {
               const data = <IMyAddonsRaw> response;
               const addons = data.addons.map(addon => new Lesson(addon));
    ...
    public getLessonsPage (pageNumber: number = 1, spaceId: number = null, order: LessonsOrder): Observable<TLessonsPage> {
    ...
    public getLessonsInBatches (observer: Observer<Lesson[]>, cursor: string = null, spaceId) {
    ...
    public getAddonsInBatches (observer: Observer<Lesson[]>, cursor: string = null, spaceId) {
    ...
    public getTrashPage (pageNumber: number = 1, spaceId: number = null): Observable<TLessonsPage> {
    ...
    public getTrashLessonsInBatches (observer: Observer<Lesson[]>, cursor: string = null, spaceId) {
    ...
    public deleteCorporateLesson(contentId: any) {
        return this._restClient.delete(MY_CONTENT_URL + "corporate/" + contentId + '/delete');
    }

    public copyLesson(contentId: any, spaceId: any) {
        return this._restClient.post(MY_CONTENT_URL + contentId + '/' + spaceId + '/copy');
    }

    public exportLesson(contentId: any, version: any) {
        return this._restClient.post(MY_CONTENT_URL + contentId + '/' + version + '/export')
    }

    public publishLesson(contentId: any) {
        return this._restClient.post(MY_CONTENT_URL + contentId + '/makepublic')
    }

    public unpublicLesson(contentId: any) {
        return this._restClient.delete(MY_CONTENT_URL + contentId + '/makepublic')
    }
    ...
    public updateTemplate(contentId: any, parameters: any) {
        return this._restClient.post(MY_CONTENT_URL + contentId + '/update_template', parameters);
    }

    public copyToAccount(contentId: any, parameters: any){
        return this._restClient.post(MY_CONTENT_URL + contentId + '/copy_to_account', parameters)
    }

    public lessonList(contentId: any, spaceId: any): Observable<{pages: MergeLesson[], commons: MergeLesson[]}> {
        return this._restClient.get(MY_CONTENT_URL + contentId + '/' + spaceId + '/page_list').pipe(
                map(response => {
                return {
                    pages: response.pages_chapters.map(page => new MergeLesson(page)),
                    commons: response.common_pages.map(page => new MergeLesson(page))
                }
            })
        );
    }
    ...
    public merge(spaceId: any, parameters: any) {
        return this._restClient.post(MY_CONTENT_URL + spaceId + '/merge', parameters);
    }

    public getEditLessonToken(): Observable<EditToken> {
        return this._restClient.get(EDIT_LESSON_TOKEN_URL).pipe(
            map(response => new EditToken(response))
        );
    }

    public getEditAddonToken(): Observable<EditToken> {
        return this._restClient.get(EDIT_ADDON_TOKEN_URL).pipe(
            map(response => new EditToken(response))
        );
    }
    ...
    public uploadAsset(contentId: any, parameters: any) {
        return this._restClient.post(MY_CONTENT_URL + contentId + '/upload_asset', parameters);
    }

    public uploadAssetPackage(contentId: any, parameters: any) {
        return this._restClient.post(MY_CONTENT_URL + contentId + '/upload_asset_package', parameters);
    }

    public setCurrentVersion(contentId: any, versionId: any) {
        return this._restClient.post(MY_CONTENT_URL + 'set_version/' + contentId + "/" + versionId);
    }
}
