...
export class BrowseLabelsPanelComponent implements OnInit {
    ...
    ngOnInit() {
        this._translationsAdmin.getLanguagesList().subscribe((langList: Language[]) => {
            ...
            if (this.languages.length) {
                ...
                for (let lang of this.realLanguages) {
                    this._translationsAdmin.getLabels(lang).subscribe(
                        (result: { labels: { [key: string]: string } }) => {
        ...
        this._translations.getTranslations().subscribe((t: ITranslations | null) => {
            if (t) {
                this.translations = t;
            }
        });
    ...
    public addLabelsToList(languageKey: string, json: { labels: { [key: string]: string } }) {
    ...
    public clickOutsideTextarea(label: Label) {
        ...
        let foundLang: Language | undefined = this.languages.find(lang => lang.key.localeCompare(label.language) === 0);
        if (!foundLang) {
    ...
    private compareLabelsField(sortObject: SortLabelsBy, a: Label, b: Label): number {
    ...
    private includes = (str: string, expr: string): boolean => (str || '').toLowerCase().indexOf(expr.toLowerCase()) !== -1;
}
