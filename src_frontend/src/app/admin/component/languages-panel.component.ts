...
export class LanguagesPanelComponent implements OnInit {
    ...
    ngOnInit() {
        forkJoin({
            lang: this._translationsAdmin.getLanguagesList(),
            ...
            const defaultLangIndex = this.languages.findIndex(
                (lang) => lang.key.localeCompare(settings.languageCode) === 0
            );
            if (defaultLangIndex !== -1) {
                const defaultLang = this.languages[defaultLangIndex];
                const updatedLang = new Language({
                    id: defaultLang.id,
                    lang_key: defaultLang.key,
                    lang_description: defaultLang.description,
                    created_date: "",
                    modified_date: "",
                });
                this.languages[defaultLangIndex] = updatedLang;
            }
    ...
    public addLanguage(): void {
        ...
        if (
            this.languages.find((elem) => elem.key === this.newLanguageKey) !==
            undefined
        ) {
            this._infoMessage.addWarning(
                "Language with this key already exists"
            );
            return;
        }
        ...
        if (this.newLanguageKey.length > 7) {
            this._infoMessage.addWarning(
                "Language key can't have more than 7 characters"
            );
            return;
        }

        const lang = Language.createNew(
            this.newLanguageKey,
        ...
        this._translationsAdmin.addLanguage(lang).subscribe(
            (response: ILanguageRaw) => {
                const newLang = new Language(response);
                ...
                this._infoMessage.addSuccess(
                    "Sent request to add new language. Please check email."
                );
            },
            (error) => {
                this._infoMessage.addError("Error while adding new language");
            }
        );
    }
    ...
    public deleteLanguage(lang: Language): void {
        ...
        this._translationsAdmin.deleteLanguage(lang).subscribe(
            (_) => {
                this.languages = this.languages.filter((iter) =>
                    iter.key.localeCompare(lang.key)
                );
        ...
        this._translationsAdmin
            ...
            .subscribe((translations: ITranslations) => {
                let generatedJson: string =
                    this._utils.generateJson(translations);
    ...
    public translationsTextboxChange(newValue: string): void {
        ...
        this.importLabelsStatus = this._utils.isJsonValid(
            this.translationsTextarea
        );
    }
    ...
    public importLanguage(): void {
        ...
        this._translationsAdmin
            ...
            .subscribe(
                (result) => {
                    this._infoMessage.addSuccess(
                        "Sent request to import labels. Please wait for email."
                    );
                },
                (error) => {
