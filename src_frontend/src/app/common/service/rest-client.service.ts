...
export class RestClientService {
    ...
    public getPublic(url: string): Observable<any> {
        return this._http.get(API_URL + url, this.getOptions()).pipe(
    ...
    private requestWithToken(
        getOptions: (header: any) => RequestOptions,
        handleError: (error: any, that: RestClientService) => Observable<any>,
        methodName: string,
        ...params: any[]
    ): Observable<any> {
        return Observable.create((observer: Observer<any>) => {
            this._token.get().subscribe((token: string) => {
                if (!token) {
                    observer.error(UNAUTHORIZED_ERROR);
                } else {
                    let options = this.getRequestOptions(
                        methodName,
                        getOptions(this.getTokenHeader(token)),
                        ...params
                    );
                    this._http
                        ...
                        .subscribe(
                            ...
                            (error) => {
                                if (
                                    ...
                                    SERVER_RESPONSE_STATUS.UNAUTHORIZED
                                ) {
                                    this._token.clear();
