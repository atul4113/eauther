...
export class BaseUploadFileComponent implements OnInit {
        ...
        return Observable.create( (observer: Observer<FileData>) => {
            if (this.observer) {
                ...
                if (this.file && !this.file.isSuccess) {
                    Observable.forkJoin(
                        this._token.getFreshToken(),
                        this._uploadFile.getUploadUrl()
                    )
                    .subscribe( ([token, uploadUrl]) => {
                        ...
                        this.observer.error(new UploadFileError('Could not get token', 401));
                        this.observer.complete();
                    });
                } else {
                    this.observer.error(new UploadFileError('File not provided or file already uploaded.', 404));
                    this.observer.complete();
                }
            }
        });
    ...
    private initUploader () {
        ...
        this.uploader.onAfterAddingFile = (item: FileItem) => {
            ...
            this.file = item;
            this.observer = null;
            this.selected.emit(new FileData(item.file.name, item.file.type, item.file.size));
        };

        this.uploader.onSuccessItem = (item: FileItem, response: string, status: number, headers: ParsedResponseHeaders) => {
            let data: {uploaded_file_id: number} = JSON.parse(response);
            this.uploader.clearQueue();
            this.observer.next(new FileData(item.file.name, item.file.type, item.file.size, data.uploaded_file_id, true));
            this.observer.complete();
            this.init();
        };
