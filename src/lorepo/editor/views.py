...
from lorepo.spaces.util import get_all_user_spaces,\
    get_space_for_content, get_private_space_for_user
import datetime
import json
...
def addons(request, content_id):
    ...
    addons_from_request_space = Content.objects.filter(spaces=str(space.id), content_type=ContentType.ADDON, is_deleted=False)
...
def _read_corporate_addons(request):
    ...
    if contents is None:
        contents = []
        spaces = list(Space.objects.filter(parent__id = request.user.company.id, space_type=SpaceType.CORPORATE )) #get all company divisions
        for space in spaces:
            if _should_include_contents_in_editor(space):
                contents.extend(Content.objects.filter(spaces=str(space.id), content_type=ContentType.ADDON, is_deleted=False, is_public=True))
...
def _read_corporate_templates(request):
    ...
    #user_templates = cache.get('templates_for_%s' % (request.user))
    user_templates = None
    if user_templates is None:
        user_templates = Content.objects.filter(spaces = str(user_space.id), content_type = ContentType.TEMPLATE, is_deleted = False).order_by('title')
    ...
    #corporate_templates = cache.get('templates_for_%s' % (request.user.company.id))
    corporate_templates = None
    if corporate_templates is None:
        corporate_templates = []
        for space in sharing_corporate_spaces:
            corporate_templates.extend( Content.objects.filter(spaces = str(space.id), content_type = ContentType.TEMPLATE, is_deleted = False).order_by('title') )
...
def _read_contents(request, content_type, is_deleted=False):
    ...
    for space in user_spaces:
        tmp_contents = get_contents_from_specific_space(space.id, lambda content: content.content_type == content_type and content.is_deleted == is_deleted)
