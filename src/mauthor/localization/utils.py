...
def create_id_string(field, module):
    id_string = '%(module_id)s:%(module_name)s|%(property_name)s' % { 'module_id' : module.id, 'module_name' : module.name, 'property_name' : field.name }
...
def create_upload_file(xliff, content_id):
    ...
    file_name = generate_unique_gcs_path(bucket, 'localization.xliff', content_id)
    store_file_from_stream(file_name, 'application/x-liff+xml', stream)

    uploaded_file = UploadedFile()
    uploaded_file.file = str(blobstore.create_gs_key('/gs' + file_name))
...
def filter_localization_properties(properties):
    return [property_element.getAttribute('name') 
            for property_element in properties 
            if property_element.getAttribute('isLocalized') == 'true' or property_element.getAttribute('type') == 'narration']
