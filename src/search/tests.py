...
class TestIndexed(TestCase):
    def setUp(self):
        ...
        Indexed(one='foo', two='bar').save()
        Indexed(one='foo_2', two='bar').save()

        for i in range(3):
            Indexed(extra_data=extra_data, one='OneOne%d' % i).save()

        for i in range(3):
            Indexed(extra_data=extra_data, one='one%d' % i, two='two%d' % i).save()

        for i in range(3):
            Indexed(extra_data=extra_data, one=(None, 'ÃœÃ„Ã–-+!#><|', 'blub')[i],
                    check=bool(i%2), value='value%d test-word' % i).save()

        for i in range(3):
            FiltersIndexed(check=bool(i%2), value='value%d test-word' % i).save()
