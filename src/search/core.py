...
def startswith(words, indexing, **kwargs):
    ...
    result = []
    for word in words:
        result.extend([word[:count].strip('-')
...
def default_splitter(text, indexing=False, **kwargs):
    ...
    if not text:
        return []
    if not indexing:
        return _PUNCTUATION_SEARCH_REGEX.sub(' ', text.lower()).split()
    keywords = []
    for word in set(_PUNCTUATION_REGEX.sub(' ', text.lower()).split()):
        if not word:
            continue
        if '-' not in word:
...
def get_word_combinations(word):
    ...
    for count in range(1, len(parts) + 1):
        for index in range(len(parts) - count + 1):
            permutations.append(''.join(parts[index:index+count]))
    return permutations
...
#        setattr(model, '_default_manager', None)
    ...
    def should_index(self, values):
        ...
        if not values:
            return False
        for filter, value in list(self.filters.items()):
