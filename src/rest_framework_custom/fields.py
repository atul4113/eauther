...
class MySerializer(serializers.Serializer):
    timestamp = serializers.DateTimeField(format="%Y-%m-%dT%H:%M:%S.%fZ", input_formats=['iso-8601'])
class empty:
    """
...
class CreateOnlyDefault(object):
    ...
    def set_context(self, serializer_field):
        ...
        if isinstance(self.default, collections.Callable) and hasattr(self.default, 'set_context') and not self.is_update:
...
class CommaSeparatedUserField(Field):
    ...
    def to_internal_value(self, value):
        ...
        if unknown_names or invalid_users:
            raise ValidationError(_("The following usernames are incorrect: %(users)s") % {'users': ', '.join(list(unknown_names)+invalid_users)})
