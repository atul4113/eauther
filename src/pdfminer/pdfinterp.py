...
class PDFPageInterpreter(object):
    ...
    # stroke
    def do_S(self):
        self.device.paint_path(self.graphicstate, True, False, False, self.curpath, self.stroke_color, self.nonstroke_color)
        self.curpath = []
        return
    ...
    # fill
    def do_f(self):
        self.device.paint_path(self.graphicstate, False, True, False, self.curpath, self.stroke_color, self.nonstroke_color)
        self.curpath = []
        return
    # fill (obsolete)
    do_F = do_f
    ...
    # fill-even-odd
    def do_f_a(self):
        self.device.paint_path(self.graphicstate, False, True, True, self.curpath, self.stroke_color, self.nonstroke_color)
        self.curpath = []
        return

    # fill-and-stroke
    def do_B(self):
        self.device.paint_path(self.graphicstate, True, True, False, self.curpath, self.stroke_color, self.nonstroke_color)
        self.curpath = []
        return

    # fill-and-stroke-even-odd
    def do_B_a(self):
        self.device.paint_path(self.graphicstate, True, True, True, self.curpath, self.stroke_color, self.nonstroke_color)
        self.curpath = []
        return
    ...
    def do_scn(self):
        if self.ncs:
            ...
            if STRICT:
                raise PDFInterpreterError('No colorspace specified!')
            n = 1
        self.nonstroke_color = self.pop(n)
        logging.error(self.nonstroke_color)
        return
        ...
        while 1:
            ...
            if isinstance(obj, PSKeyword):
                ...
                if hasattr(self, method):
                    ...
                    if nargs:
                        args = self.pop(nargs)
                        #logging.error('exec: %s %r', name, args)
                        if len(args) == nargs:
                            func(*args)
                    else:
                        #logging.error('exec: %s', name)
                        func()
                else:
                    if STRICT:
