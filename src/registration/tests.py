...
class RegistrationModelTests(RegistrationTestCase):
    ...
    def test_activation(self):
        ...
        self.assertEqual(RegistrationProfile.objects.activate_user(RegistrationProfile.objects.get(user=self.sample_user).activation_key).pk,
                             self.sample_user.pk)
        
        # The activated user must now be active.
        self.assertTrue(User.objects.get(pk=self.sample_user.pk).is_active)
        
        # The activation key must now be reset to the "already activated" constant.
        self.assertEqual(RegistrationProfile.objects.get(user=self.sample_user).activation_key,
                             RegistrationProfile.ACTIVATED)
        
        # Activating an expired user returns False.
        self.assertFalse(RegistrationProfile.objects.activate_user(RegistrationProfile.objects.get(user=self.expired_user).activation_key))
        ...
        self.assertFalse(RegistrationProfile.objects.activate_user(sha.new('foo').hexdigest()))
    ...
    def test_account_expiration_condition(self):
        ...
        self.assertFalse(RegistrationProfile.objects.get(user=self.sample_user).activation_key_expired())

        # Expired user returns True.
        self.assertTrue(RegistrationProfile.objects.get(user=self.expired_user).activation_key_expired())
        ...
        self.assertTrue(RegistrationProfile.objects.get(user=self.sample_user).activation_key_expired())
...
class RegistrationFormTests(RegistrationTestCase):
    ...
    def test_registration_form(self):
        ...
        invalid_data_dicts = [
            ...
            { 'username': 'foo',
              ...
              'password2': 'bar' },
            'error':
            ('__all__', ["You must type the same password each time"])
            },
            ]
    ...
    def test_registration_form_tos(self):
        ...
        form = forms.RegistrationFormTermsOfService(data={ 'username': 'foo',
                                                           ...
                                                           'password2': 'foo' })
        self.assertFalse(form.is_valid())
        self.assertEqual(form.errors['tos'], ["You must agree to the terms to register"])
    ...
    def test_registration_form_unique_email(self):
        ...
        form = forms.RegistrationFormUniqueEmail(data={ 'username': 'foo',
                                                        ...
                                                        'password2': 'foo' })
        self.assertFalse(form.is_valid())
        self.assertEqual(form.errors['email'], ["This email address is already in use. Please supply a different email address."])
    ...
    def test_registration_form_no_free_email(self):
        ...
        for domain in ('aim.com', 'aol.com', 'email.com', 'gmail.com',
            ...
            self.assertFalse(form.is_valid())
            self.assertEqual(form.errors['email'], ["Registration using free email addresses is prohibited. Please supply a different email address."])
...
class RegistrationViewTests(RegistrationTestCase):
    ...
    def test_activation_view(self):
        ...
        self.assertFalse(response.context['account'])
